...
data=${stream}
pg="2/m_uab" #"4/mmm" #"2/m" #"mmm"
pdb=$2 #/gpfs/cfel/group/cxi/scratch/2021/LCLS-2024-Meents-Oct-MFX/processed/results/lassa/all/mtz/lassa.cell #/gpfs/cfel/group/cxi/scratch/2021/LCLS-2024-Meents-Oct-MFX/processed/results/fakp/cell_files/${stream}.cell
echo $pdb
max_adu="" #"--max-adu=2000"
HIGHRES=1.9
...
module load maxwell crystfel python/3.9
    # Job name
    NAME=$data
...
#    echo "#SBATCH --partition=cfel" >> $SLURMFILE  # Set your partition here
    ...
    echo "#SBATCH --time=72:00:00" >> $SLURMFILE
...
#    echo "#SBATCH --workdir   $PWD" >> $SLURMFILE
	...
	echo "module load maxwell crystfel python/3.9">> $SLURMFILE
    ...
    #command="$partialator --no-logs --max-adu=7500 --iterations=$iterations --model=$model "$maxB" "$minres" "$push" -i $stream.stream -o $data.hkl -y $pg -j 80"
    command="$partialator --no-logs --iterations=$iterations --model=$model "$max_adu" "$minres" "$push" -i $stream.stream -o $data.hkl -y $pg -j 120"
...
#    echo $highres1
	...
	echo $max_dd
	#pwd
	command="python3 /gpfs/cfel/group/cxi/scratch/2020/EXFEL-2019-Schmidt-Mar-p002450/scratch/galchenm/scripts_for_work/plot_func/many_plots-upt-v2.py -i ${data}_CCstar.dat -x '1/d' -y 'CC*' -o ${data}_cut_off_1p5.png -add_nargs ${data}_Rsplit.dat -yad 'Rsplit/%' -x_lim_dw 1. -x_lim_up ${max_dd} -t ${data} -legend ${data} >> $ERRORDIR/output.err"
	echo $command >> $SLURMFILE
	
	command="python3 /gpfs/cfel/group/cxi/scratch/2020/EXFEL-2019-Schmidt-Mar-p002450/scratch/galchenm/scripts_for_work/update_peakogram/avr-peakogram-stream -i $stream.stream"
	echo $command >> $SLURMFILE
	
	
	command="python3 /gpfs/cfel/group/cxi/scratch/2021/LCLS-2024-Meents-Oct-MFX/processed/results/orientation-v2.py $stream.stream"
	echo $command >> $SLURMFILE	
    sbatch $SLURMFILE
